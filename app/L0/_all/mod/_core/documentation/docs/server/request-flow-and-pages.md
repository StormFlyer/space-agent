...
- when `LOGIN_ALLOWED=false`, `/login` keeps the public shell and outbound links but swaps the form for a visible `Login is disabled in this system.` message; background guest-session issuance and session-check flows may still run outside that form
...
- after a successful clone, the public share shell waits for the new guest session to become visible through `login_check`, then grants the same-tab launcher-access marker before navigating so the imported guest space opens directly instead of bouncing back through `/enter` or `/login`
