...
- `/enter` reuses that same navigation-only footer row of local SVG icon links to `https://github.com/agent0ai/space-agent`, `https://discord.gg/B8KZKNsPpj`, `https://agent-zero.ai`, and `https://x.com/Agent0ai`, and in packaged Electron runs it may also run a recovery-safe background update check, reveal an update button below `Admin Mode` only when the preload bridge reports an available or downloaded update, keep normal update status inside that button label with no second text line underneath, stay visually quiet when no newer bundled release exists, collapse update-check failures behind a `Could not check updates` disclosure with expandable raw details, and display redundant updater versions such as `0.44.0` as the two-segment form `0.44`
