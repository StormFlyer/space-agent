...
- open, navigate, history, reload, `state(...)`, `content(...)`, `detail(...)`, and `dom(...)` settle internally and return fresh read results without a separate sync step
...
- inspection: dom(id, payload?), content(id, payload?), detail(id, referenceId)
