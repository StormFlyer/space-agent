...
- `web_browsing/`: floating browser-overlay module that contributes a Browser dropdown action and mounts draggable, minimizable, resizable browser windows with a toolbar-driven `<x-browser>` shell that uses an iframe fallback in browser sessions and a DOM-backed desktop `<webview>` in the packaged host, including in-app interception of `_blank` or `window.open(...)` requests back into new `browser-N` modals, a top-level numeric-id `space.browser` runtime surface for agent work, persistent browser-local window restore across reloads, focus-to-front stacking when a browser window or its inner page surface is clicked, and app-side browser diagnostics gated by a shared browser log level that defaults to `error`
